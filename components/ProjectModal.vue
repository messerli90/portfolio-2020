<template>
  ...
  >
    ...
    >
      <div v-show="isOpen" class="fixed inset-0 transition-opacity" @click="closeModal">
    ...
    >
      ...
      >
        <div class="relative bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 flex flex-col">
          <button
            class="absolute top-0 right-0 m-4 p-2 flex justify-center items-center rounded-full bg-gray-900 text-gray-200 z-10 opacity-75 transition transform duration-200 ease-in-out hover:scale-105 hover:opacity-100"
            @click="closeModal"
          >
          ...
          <client-only v-if="project.images" placeholder="Loading...">
            <!-- <agile class="rounded-md overflow-hidden">
              <div v-for="(img, i) in project.images" :key="i" class="relative slide">
                <div class="h-100 text-center">
                  <img :src="img.src" :alt="img.alt" class="h-100 object-center" />
                </div>
                <p
          ...
          <h4 class="my-2 text-xl font-medium text-indigo-600">{{ project.title }}</h4>
